...
static int atmel_power_source_ctrl(struct mxt_data *data, int enable)
{
	...
	if (enable) {
		...
		if (!IS_ERR_OR_NULL(data->vcc_i2c)) {
			...
			if (rc)
				CTP_ERROR("Regulator vcc_i2c disable failed rc=%d", rc);
		}
	}
	return 0;
}

static void atmel_power_source_exit(struct mxt_data *data)
{
	if (!IS_ERR_OR_NULL(data->vdd)) {
		if (regulator_count_voltages(data->vdd) > 0)
			regulator_set_voltage(data->vdd, 0, ATMEL_VTG_MAX_UV);
...
#if ATMEL_POWER_SOURCE_CUST_EN
	atmel_power_source_ctrl(data, 0);
	atmel_power_source_exit(data);
#endif
/*
err_free_regulator:
