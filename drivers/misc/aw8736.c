...
** Modified for AW8736 by Tamas Kemenesi
...
static ssize_t aw8736_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", value);
}

static ssize_t aw8736_store(struct device *dev,
...
static DEVICE_ATTR(aw8736_amp_enable, 0664, aw8736_show, aw8736_store);

static struct attribute *aw8736_amp_attributes[] =
{
	&dev_attr_aw8736_amp_enable.attr,
	NULL
};	

static struct attribute_group aw8736_amp_group =
{
	.attrs  = aw8736_amp_attributes,
};

static struct miscdevice aw8736_amp_device =
{
	.minor = MISC_DYNAMIC_MINOR,
	.name = "aw8736_amp",
};

static int __init aw8736_amp_init(void)
{
	int ret;

	pr_info("%s misc_register(%s)\n", __FUNCTION__,
		aw8736_amp_device.name);

	ret = misc_register(&aw8736_amp_device);
	...
	if (sysfs_create_group(&aw8736_amp_device.this_device->kobj,
			&aw8736_amp_group) < 0) {
