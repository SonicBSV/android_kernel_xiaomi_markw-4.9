...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_SYSCTL)
static int mxt_ops_enable_dt2w(struct device *dev, bool enable)
{
	struct mxt_data *data = dev_get_drvdata(dev);
	const struct mxt_platform_data *pdata = data->pdata;
	int index = data->current_index;
...
static int mxt_ops_disable_keys(struct device *dev, bool disable)
{
	struct mxt_data *data = dev_get_drvdata(dev);
...
static struct xiaomi_touchscreen_operations_t mxt_ts_ops = {
	.enable_dt2w = mxt_ops_enable_dt2w,
	.disable_keys = mxt_ops_disable_keys,
};
#endif
...
#if IS_ENABLED(CONFIG_TOUCHSCREEN_SYSCTL)
	mxt_ts_ops.dev = &client->dev;
	xiaomi_touchscreen_register_operations(&mxt_ts_ops);
#endif
